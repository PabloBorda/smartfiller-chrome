...
$("input:not([class*='search']):not([class*='chat'])[type!='hidden'],textarea").each(
	...
	/*   
	   chrome.extension.onRequest.addListener(function(request, sender, sendResponse) {
	   	 ...
	   	 if (req.type=="retrieve_entries"){
	   	   var formEntries = selectFormEntries();
	          sendResponse(formEntries);
			  console.log("Response from content script: " + JSON.stringify(formEntries));
	   	 } else {
  ...
  */
     ...
     $("form").submit(function(){
        ...
        var formEntries = selectFormEntries();
        chrome.extension.sendRequest({"type":"save_entries","entries": formEntries },function(response){
	...
	chrome.extension.sendRequest({"save": formEntries } ,
	    ...
	    //while (response==undefined){};
            console.log("From contentscript save form data" + JSON.stringify(response));  
  ...
  chrome.extension.sendRequest({ "tofill": formEntries } ,
    //chrome.extension.sendRequest(document.title ,
     function(response){
       //while (response==undefined){};
       console.log("From contentscript tofill got response: " + JSON.stringify(response));
